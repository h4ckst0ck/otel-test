...
func (r *otlpReceiver) startProtocolServers(host component.Host) error {
	...
	var f MQTT.MessageHandler = func(client MQTT.Client, msg MQTT.Message) {
		...
		if strings.HasPrefix(msg.Topic(), "opentelemetry.proto.collector.trace") {
			otlpReq, err := pbEncoder.unmarshalTracesRequest(msg.Payload())
			if err != nil {
				//writeError(resp, encoder, err, http.StatusBadRequest)
				return
			}

			otlpResp, err := r.traceReceiver.Export(context.Background(), otlpReq)
			if err != nil {
				//writeError(resp, encoder, err, http.StatusInternalServerError)
				return
			}
			otlpResp.MarshalJSON()
		}

		if strings.HasPrefix(msg.Topic(), "opentelemetry.proto.collector.logs") {
			otlpReq, err := pbEncoder.unmarshalLogsRequest(msg.Payload())
			if err != nil {
				//writeError(resp, encoder, err, http.StatusBadRequest)
				return
			}

			otlpResp, err := r.logReceiver.Export(context.Background(), otlpReq)
			if err != nil {
				//writeError(resp, encoder, err, http.StatusInternalServerError)
				return
			}
	...
	opts := MQTT.NewClientOptions().AddBroker("tcp://localhost:18833")
	...
	if token := client.Subscribe("opentelemetry.proto.collector.trace.v1.ExportTraceServiceRequest/#", 0, nil); token.Wait() && token.Error() != nil {
		return token.Error()
	}

	if token := client.Subscribe("opentelemetry.proto.collector.logs.v1.ExportLogsServiceRequest/#", 0, nil); token.Wait() && token.Error() != nil {
		return token.Error()
	}
